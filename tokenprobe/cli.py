"""
Command-line interface for tokenprobe.
...
from tokenprobe import __version__
...
from tokenprobe.logging_config import PhaseLogger, setup_logging
...
@click.version_option(version=__version__, prog_name="tokenprobe")
def main(
    token: str | None,
    output_json: bool,
    verbose: bool,
    ...
    if batch:
        ...
        from tokenprobe.core.batch import load_tokens_from_file, process_batch, save_batch_result
    ...
    if token is None:
        if sys.stdin.isatty():
            ...
            console.print("Run [bold]tokenprobe --help[/bold] for usage.")
    ...
    try:
        ...
        from tokenprobe.logging_config import ErrorLogger
